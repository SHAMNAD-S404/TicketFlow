...
export class AuthService implements IAuthService {
    ...
    async registerUser(data:RegisterUserDTO) : Promise<{message:string,success:boolean}> {
        ...
        try {
            ...
            const role = "company";
            const storeUser = await this.userRepository.create(email,hashedPassword,role);
            if(!storeUser){
     ...
     */
    async verifyLogin(email: string, password: string): Promise<{ message: string; success: boolean; tockens?:{refreshToken : string,accessToken:string}}> {
        try {
            ...
            return {message:"Login Successfull",success:true,tockens:{accessToken,refreshToken}}
    ...
    async verifyEmail(email: string): Promise<{ message: string; success: boolean; }> {
        try {
            ...
            const existingTempUser = await getRedisData(`verifyEmail:${email}`);
            if(existingTempUser){
                return {message: 'User registration already in progress',success:false}
            }
